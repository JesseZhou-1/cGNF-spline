...
def simulate_and_save(path, dataset_name, N=20000, seed=0):
    np.random.seed(seed)
    # Simulate continuous normal C and continuous A, L, M, Y (all Gaussian noises)
    loc_c, scale_c = 0, 1
    C = np.random.normal(loc_c, scale_c, N)

    # treatment, normal
    loc_a, scale_a = 0, 1
    A = np.random.normal(loc_a, scale_a, N) + 0.1 * C

    # exposure-induced confounder, normal
    loc_l, scale_l = 0, 1
    L = np.random.normal(loc_l, scale_l, N) + 0.2 * A + 0.2 * C

    # mediator, normal
    loc_m, scale_m = 0, 1
    M = np.random.normal(loc_m, scale_m, N) + 0.1 * A + 0.2 * C + 0.25 * L

    # outcome, normal
    loc_y, scale_y = 0, 1
    Y = np.random.normal(loc_y, scale_y, N) + 0.1 * A + 0.1 * C + 0.2 * M + 0.25 * L
